...
//uni-ui弹出层组件ref  这是利用了vue3的组合式API特性,不需要想之前vue2的写法那样
const popup = ref<{
  //声明各类型,方便使用
...
//SKU模块按钮模式 (选择或者加入购物车或者直接购买的那个弹出框对应样式)
enum SkuMode {
  /** Both:都显示 */
  Both = 1,
  /**Cart:只显示购物车  */
...
const getGoodsByIdData = async () => {
  ...
  //       price: v.price * 100,
  //       stock: v.inventory,
  //       sku_name_arr: v.specs.map((va) => va.valueName),
  //     }
  //   }),
  // }
}
...
const addCart = async () => {
  console.log('触发加入购物车')
  await postMemberCartAPI({ goodsId: goods.value!.id, count: 1 })
  uni.showToast({ icon: 'none', title: '添加成功' })
}
//立即购买,非sku模块
const buyNow = () => {
  ...
  //跳转到订单界面
  uni.navigateTo({
    url: `/pagesOrder/create/create?goodsId=${goods.value?.id}&count=1&addressId=${selectAddress.value?.id}`,
  })
}
//立即购买
    ...
    <view class="goods">
      ...
      <view class="action">
        ...
        <view class="item arrow" @tap="openPopup('address')">
          ...
          <text v-if="selectAddress" class="text ellipsis">
